...
def fix_contractions(text: str) -> str:
    """Fix broken contractions where apostrophe was separated.

    Only handles the most common, unambiguous contractions.

    Args:
        text: Text with potential broken contractions

    Returns:
        Text with contractions restored
    """
    # Common contraction patterns: word + space + suffix → word'suffix
    contraction_patterns = [
        # 's contractions (most common)
        (r"\b(that)\s+(s)\b", r"\1'\2"),
        (r"\b(what)\s+(s)\b", r"\1'\2"),
        (r"\b(it)\s+(s)\b", r"\1'\2"),
        (r"\b(there)\s+(s)\b", r"\1'\2"),
        (r"\b(here)\s+(s)\b", r"\1'\2"),
        (r"\b(let)\s+(s)\b", r"\1'\2"),
        # 've contractions
        (r"\b(you)\s+(ve)\b", r"\1'\2"),
        (r"\b(we)\s+(ve)\b", r"\1'\2"),
        (r"\b(they)\s+(ve)\b", r"\1'\2"),
        (r"\b(I)\s+(ve)\b", r"\1'\2"),
        # 're contractions
        (r"\b(you)\s+(re)\b", r"\1'\2"),
        (r"\b(we)\s+(re)\b", r"\1'\2"),
        (r"\b(they)\s+(re)\b", r"\1'\2"),
        # 'll contractions
        (r"\b(you)\s+(ll)\b", r"\1'\2"),
        (r"\b(we)\s+(ll)\b", r"\1'\2"),
        (r"\b(they)\s+(ll)\b", r"\1'\2"),
        (r"\b(I)\s+(ll)\b", r"\1'\2"),
        # 't contractions (negatives)
        (r"\b(don)\s+(t)\b", r"\1'\2"),
        (r"\b(doesn)\s+(t)\b", r"\1'\2"),
        (r"\b(didn)\s+(t)\b", r"\1'\2"),
        (r"\b(won)\s+(t)\b", r"\1'\2"),
        (r"\b(can)\s+(t)\b", r"\1'\2"),
        (r"\b(couldn)\s+(t)\b", r"\1'\2"),
        (r"\b(wouldn)\s+(t)\b", r"\1'\2"),
        (r"\b(shouldn)\s+(t)\b", r"\1'\2"),
        (r"\b(isn)\s+(t)\b", r"\1'\2"),
        (r"\b(aren)\s+(t)\b", r"\1'\2"),
        (r"\b(wasn)\s+(t)\b", r"\1'\2"),
        (r"\b(weren)\s+(t)\b", r"\1'\2"),
        # 'm contraction
        (r"\b(I)\s+(m)\b", r"\1'\2"),
        # 'd contractions
        (r"\b(I)\s+(d)\b", r"\1'\2"),
...
def decode_ligatures(text: str) -> str:
    """Decode PDF ligatures - conservative approach.

    Only handles unambiguous transformations:
    1. Unicode ligatures (U+FB00-U+FB06) → component letters
    2. Common broken contractions

    Does NOT attempt:
    - Word splitting/merging (too error-prone)
    - Corrupted ligature guessing (causes false positives)
    - Case boundary detection (interferes with ligature markers)
    ...
    """
    # Handle proper Unicode ligatures (unambiguous)
